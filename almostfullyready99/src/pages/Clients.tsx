...
const Clients = (props: any) => {
  ...
  function getCustomers() {
    axios({
      ...
      if (searchTerm != "") {
        setShowSearch(
          `${customers.length} ${t("results_for")} "${searchTerm}"`
        );
      } else {
        setShowSearch(t("displaying_all_customers"));
      }
    });
  }
  ...
  function addCustomer(e: any) {
    ...
    axios({
      ...
      .catch((error) => {
        toast.error(t("something_went_wrong")); // update this to say if customer already exists
        clearFields();
        if (error.response) {
    ...
    <>
      ...
      <div className="flex justify-center w-full min-h-screen">
        <div className="w-[80%] max-w-[900px] text-center">
          ...
          <p className="text-white pt-2 text-2xl">{t("search_clients")}</p>
